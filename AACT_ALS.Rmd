...
### Introduction
Clinical trial participation retention directly affects the generalizability and validity of the results of that trial. The primary goal of this notebook is to use a data driven approach to investigate the predictive features leading to patient attrition in als clinical trials. The data source is aggregated at the trial level. 

#### als or Major Depressive Disorder
Create list of nct_ids that have some form of "Amyotrophic Lateral Sclerosis" or ALS" listed as downcase_name of the study from the conditions table. Found 677 studies that matched the syntax below.  

##### Method for getting als studies :  
  - Table : ctgov.conditions   
  - Column : downcase_name  
    - Strings Pulled : 'als', '%amyotrophic lateral sclerosis%'  
    
###### als study key method
```{sql connection=con}
DROP TABLE IF EXISTS als_study_key_V2;
```
...
WITH als_studies AS (
  ...
  WHERE downcase_name LIKE '% als %' OR downcase_name LIKE '%amyotrophic lateral sclerosis%' OR downcase_name LIKE 'als'
  ), enrollment_info AS ( 
...
SELECT als_studies.nct_id, enrollment_total, drop_count_total
INTO TEMP als_study_key_V2
FROM als_studies 
LEFT JOIN enrollment_info
ON als_studies.nct_id = enrollment_info.nct_id
LEFT JOIN dropout_info
ON als_studies.nct_id = dropout_info.nct_id
```

```{sql connection=con}
SELECT COUNT(DISTINCT nct_id)
FROM als_study_key_V2;
```    

## create table with ae nct_ids, subject_count, and ids for creating adverse events table
```{sql connection=con, output.var="als_ae_key"}
WITH als_studies AS (
  SELECT *
  FROM ctgov.conditions 
  WHERE downcase_name LIKE '% als %' OR downcase_name LIKE '%amyotrophic lateral sclerosis%' OR downcase_name LIKE 'als'
...
SELECT als_studies.nct_id, reported_events.id, subjects_affected
FROM als_studies 
LEFT JOIN reported_events
ON als_studies.nct_id = reported_events.nct_id;
```

```{r}
write.csv(als_ae_key, "/Users/nataliecortopassi/Library/Mobile Documents/com~apple~CloudDocs/510 - HIDS/Adverse Events/ae_key_dfs/als_ae_key.csv")
```    
...
```{sql connection=con}
DROP TABLE IF EXISTS als_event_table;
```
...
WITH event_info AS (
    ...
    SELECT als_study_key_V2.nct_ID
    FROM als_study_key_V2
)
 SELECT als_key.nct_id, subjects_affected_total, event_type
 INTO TEMP als_event_table
 FROM als_key 
 LEFT JOIN event_info
 ON als_key.nct_id = event_info.nct_id
``` 

```{sql connection=con, output.var="als_event_table"}
SELECT * 
FROM als_event_table;
```

```{r}
write.csv(als_event_table, "/Users/nataliecortopassi/Library/Mobile Documents/com~apple~CloudDocs/510 - HIDS/als_df/als_event_table.csv")
```
    
#### Select Features and Columns to create target outcome (percentage dropped) - V2
Create table that matches the nct_id found above (als studies) and add features from other aact tables
...
```{sql connection=con}
DROP TABLE IF EXISTS V2_als;
```

```{sql connection=con}
WITH als_list_key AS (
  SELECT *
  FROM als_study_key_V2 
...
SELECT als_list_key.nct_id, als_list_key.enrollment_total, als_list_key.drop_count_total, study_info.study_type, study_info.phase AS study_phase, study_info.number_of_arms, study_info.has_expanded_access, study_info.has_dmc, calculated_info.number_of_facilities,  calculated_info.actual_duration AS study_duration_months, calculated_info.number_of_sae_subjects, calculated_info.minimum_age_num, design_info.intervention_model AS intervention_model_type, design_info.allocation AS allocation_type, design_info.masking AS masking_type, eligibility_info.gender AS study_gender_eligibility, eligibility_info.healthy_volunteers AS health_status_eligibility
    INTO TEMP V2_als
    FROM als_list_key
    LEFT JOIN enrolled_count_list
    ON als_list_key.nct_id = enrolled_count_list.nct_id
    LEFT JOIN dropped_count_list
    ON als_list_key.nct_id = dropped_count_list.nct_id
    LEFT JOIN study_info
    ON als_list_key.nct_id = study_info.nct_id
    LEFT JOIN calculated_info
    ON als_list_key.nct_id = calculated_info.nct_id
    LEFT JOIN country_info
    ON als_list_key.nct_id = country_info.nct_id
    LEFT JOIN design_info
    ON als_list_key.nct_id = design_info.nct_id
    LEFT JOIN eligibility_info
    ON als_list_key.nct_id = eligibility_info.nct_id
    LEFT JOIN intervention_info
    ON als_list_key.nct_id = intervention_info.nct_id
    LEFT JOIN reported_events_info
    ON als_list_key.nct_id = reported_events_info.nct_id
    GROUP BY als_list_key.nct_id, enrollment_total, drop_count_total, study_type, study_phase, number_of_arms, has_expanded_access, has_dmc, number_of_facilities, study_duration_months, number_of_sae_subjects, minimum_age_num, intervention_model_type, allocation_type, masking_type, study_gender_eligibility, health_status_eligibility;
```

```{sql connection=con , output.var="V2_als"}
SELECT * 
FROM V2_als;
```

```{r}
write.csv(V2_als, "/Users/nataliecortopassi/Library/Mobile Documents/com~apple~CloudDocs/510 - HIDS/als_df/V2_als.csv")
```   

#### Create treatment_type table
```{sql connection=con, output.var="als_treatment_type"}
SELECT V2_als.nct_id, interventions.intervention_type
FROM V2_als
LEFT JOIN ctgov.interventions
ON V2_als.nct_id = interventions.nct_id
```
...
```{r}
write.csv(als_treatment_type, "/Users/nataliecortopassi/Library/Mobile Documents/com~apple~CloudDocs/510 - HIDS/als_df/als_treatment_type.csv")
```    
    
   
#### Select dropout reasons
Create table of dropout reasons with nct_id associated with als studies
```{sql connection=con, output.var="als_dropout_reason"}
SELECT drop_withdrawals.reason AS dropout_reason, V2_als.nct_id
FROM V2_als
LEFT JOIN ctgov.drop_withdrawals
ON V2_als.nct_id = drop_withdrawals.nct_id;
```
    
```{r}
write.csv(als_dropout_reason, "/Users/nataliecortopassi/Library/Mobile Documents/com~apple~CloudDocs/510 - HIDS/als_df/als_dropout_reason.csv")
```    
