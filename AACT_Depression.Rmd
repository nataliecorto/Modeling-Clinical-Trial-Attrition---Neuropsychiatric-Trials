...
### Introduction
Clinical trial participation retention directly affects the generalizability and validity of the results of that trial. The primary goal of this notebook is to use a data driven approach to investigate the predictive features leading to patient attrition in Depression clinical trials. The data source is aggregated at the trial level. 

#### Depression or Major Depressive Disorder
Create list of nct_ids that have some form of "depression" listed as downcase_name of the study from the conditions table. Found 5,156 studies that matched the syntax below.  

##### Method for getting depression studies :  
  - Table : ctgov.conditions   
  - Column : downcase_name  
    - Strings Pulled : 'depression', 'major depression', 'major depressive disorder%', '%major depressive disorder', 'treatment%resistant depression', 'unipolar depression'  
    
###### Depression study key method
```{sql connection=con}
DROP TABLE IF EXISTS depression_study_key_V2;
```

```{sql connection=con}
WITH depression_studies AS (
  SELECT *
  FROM ctgov.conditions 
  WHERE downcase_name LIKE 'depression'OR downcase_name LIKE 'major depression' OR downcase_name LIKE 'major depressive disorder%' OR downcase_name LIKE '%major depressive disorder' OR downcase_name LIKE 'treatment%resistant depression' OR downcase_name LIKE 'unipolar depression'
  ), enrollment_info AS ( 
...
SELECT depression_studies.nct_id, enrollment_total, drop_count_total
INTO TEMP depression_study_key_V2
FROM depression_studies 
LEFT JOIN enrollment_info
ON depression_studies.nct_id = enrollment_info.nct_id
LEFT JOIN dropout_info
ON depression_studies.nct_id = dropout_info.nct_id
```

```{sql connection=con}
SELECT COUNT(DISTINCT nct_id)
FROM depression_study_key_V2;
```    

## create table with ae nct_ids, subject_count, and ids for creating adverse events table
```{sql connection=con, output.var="depression_ae_key"}
WITH depression_studies AS (
  SELECT *
  FROM ctgov.conditions 
  WHERE downcase_name LIKE 'depression'OR downcase_name LIKE 'major depression' OR downcase_name LIKE 'major depressive disorder%' OR downcase_name LIKE '%major depressive disorder' OR downcase_name LIKE 'treatment%resistant depression' OR downcase_name LIKE 'unipolar depression'
...
SELECT depression_studies.nct_id, reported_events.id, subjects_affected
FROM depression_studies 
LEFT JOIN reported_events
ON depression_studies.nct_id = reported_events.nct_id;
```

```{r}
write.csv(depression_ae_key, "/Users/nataliecortopassi/Library/Mobile Documents/com~apple~CloudDocs/510 - HIDS/Adverse Events/ae_key_dfs/depression_ae_key.csv")
```    
...
```{sql connection=con}
DROP TABLE IF EXISTS depression_event_table;
```
...
WITH event_info AS (
    ...
    SELECT depression_study_key_V2.nct_ID
    FROM depression_study_key_V2
)
 SELECT depression_key.nct_id, subjects_affected_total, event_type
 INTO TEMP depression_event_table
 FROM depression_key 
 LEFT JOIN event_info
 ON depression_key.nct_id = event_info.nct_id
``` 

```{sql connection=con, output.var="depression_event_table"}
SELECT * 
FROM depression_event_table;
```

```{r}
write.csv(depression_event_table, "/Users/nataliecortopassi/Library/Mobile Documents/com~apple~CloudDocs/510 - HIDS/depression_df/depression_event_table.csv")
```
...
```{sql connection=con}
DROP TABLE IF EXISTS V2_depression;
```

```{sql connection=con}
WITH depression_list_key AS (
  SELECT *
  FROM depression_study_key_V2 
...
SELECT depression_list_key.nct_id, depression_list_key.enrollment_total, depression_list_key.drop_count_total, study_info.study_type, study_info.phase AS study_phase, study_info.number_of_arms, study_info.has_expanded_access, study_info.has_dmc, calculated_info.number_of_facilities,  calculated_info.actual_duration AS study_duration_months, calculated_info.number_of_sae_subjects, calculated_info.minimum_age_num, design_info.intervention_model AS intervention_model_type, design_info.allocation AS allocation_type, design_info.masking AS masking_type, eligibility_info.gender AS study_gender_eligibility, eligibility_info.healthy_volunteers AS health_status_eligibility
    INTO TEMP V2_depression
    FROM depression_list_key
    LEFT JOIN enrolled_count_list
    ON depression_list_key.nct_id = enrolled_count_list.nct_id
    LEFT JOIN dropped_count_list
    ON depression_list_key.nct_id = dropped_count_list.nct_id
    LEFT JOIN study_info
    ON depression_list_key.nct_id = study_info.nct_id
    LEFT JOIN calculated_info
    ON depression_list_key.nct_id = calculated_info.nct_id
    LEFT JOIN country_info
    ON depression_list_key.nct_id = country_info.nct_id
    LEFT JOIN design_info
    ON depression_list_key.nct_id = design_info.nct_id
    LEFT JOIN eligibility_info
    ON depression_list_key.nct_id = eligibility_info.nct_id
    LEFT JOIN intervention_info
    ON depression_list_key.nct_id = intervention_info.nct_id
    LEFT JOIN reported_events_info
    ON depression_list_key.nct_id = reported_events_info.nct_id
    GROUP BY depression_list_key.nct_id, enrollment_total, drop_count_total, study_type, study_phase, number_of_arms, has_expanded_access, has_dmc, number_of_facilities, study_duration_months, number_of_sae_subjects, minimum_age_num, intervention_model_type, allocation_type, masking_type, study_gender_eligibility, health_status_eligibility;
```

```{sql connection=con , output.var="V2_depression"}
SELECT * 
FROM V2_depression;
```

```{r}
write.csv(V2_depression, "/Users/nataliecortopassi/Library/Mobile Documents/com~apple~CloudDocs/510 - HIDS/depression_df/V2_depression.csv")
```   

#### Create treatment_type table
```{sql connection=con, output.var="depression_treatment_type"}
SELECT V2_depression.nct_id, interventions.intervention_type
FROM V2_depression
LEFT JOIN ctgov.interventions
ON V2_depression.nct_id = interventions.nct_id
```
...
```{r}
write.csv(depression_treatment_type, "/Users/nataliecortopassi/Library/Mobile Documents/com~apple~CloudDocs/510 - HIDS/depression_df/depression_treatment_type.csv")
```    

#### Select dropout reasons
Create table of dropout reasons with nct_id associated with depression studies
```{sql connection=con, output.var="depression_dropout_reason"}
SELECT drop_withdrawals.reason AS dropout_reason, V2_depression.nct_id
FROM V2_depression
LEFT JOIN ctgov.drop_withdrawals
ON V2_depression.nct_id = drop_withdrawals.nct_id;
```
    
```{r}
write.csv(depression_dropout_reason, "/Users/nataliecortopassi/Library/Mobile Documents/com~apple~CloudDocs/510 - HIDS/depression_df/depression_dropout_reason.csv")
```    
    
## Adverse Events Table 
Add the adverse event count to the V2_depression table 
```{sql connection=con}
SELECT * 
FROM reported_events;
```

```{sql connection=con, output.var="depression_reported_events"}
SELECT V2_depression.nct_id, reported_events.id, reported_events.subjects_affected
FROM V2_depression
LEFT JOIN ctgov.reported_events
ON V2_depression.nct_id = reported_events.nct_id
```

Download adverse evetns table and do a left merge with the V2_depression table 
```{r}
adverse_df <- read_csv("~/Library/Mobile Documents/com~apple~CloudDocs/510 - HIDS/master_df/final_ae.csv")
```

```{r}
library(dplyr)
left_join(depression_reported_events, adverse_df, by = 'id')
```
