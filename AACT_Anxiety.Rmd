...
### Introduction
Clinical trial participation retention directly affects the generalizability and validity of the results of that trial. The primary goal of this notebook is to use a data driven approach to investigate the predictive features leading to patient attrition in Anxiety clinical trials. The data source is aggregated at the trial level. 

#### Anxiety or Major Depressive Disorder
Create list of nct_ids that have some form of "anxiety" listed as downcase_name of the study from the conditions table. Found 3,401 studies that matched the syntax below.  

##### Method for getting Anxiety studies :  
  - Table : ctgov.conditions   
  - Column : downcase_name  
    - Strings Pulled : '%anxiety%'  
    
###### Anxiety study key method
```{sql connection=con}
DROP TABLE IF EXISTS anxiety_study_key_V2;
```

```{sql connection=con}
WITH anxiety_studies AS (
  SELECT *
  FROM ctgov.conditions 
  WHERE downcase_name LIKE '%anxiety%' AND downcase_name NOT LIKE 'anxiety%dental' AND downcase_name NOT LIKE 'anxiety reduction, elective cesarean section'
  AND downcase_name NOT LIKE 'heart rate variability, biofeedback, nicotine dependence, nicotine withdrawal, anxiety, depression, insomnia'
  ), enrollment_info AS ( 
...
SELECT anxiety_studies.nct_id, enrollment_total, drop_count_total
INTO TEMP anxiety_study_key_V2
FROM anxiety_studies 
LEFT JOIN enrollment_info
ON anxiety_studies.nct_id = enrollment_info.nct_id
LEFT JOIN dropout_info
ON anxiety_studies.nct_id = dropout_info.nct_id
```

## create table with ae nct_ids, subject_count, and ids for creating adverse events table
```{sql connection=con, output.var="anxiety_ae_key"}
WITH anxiety_studies AS (
  SELECT *
  FROM ctgov.conditions 
  WHERE downcase_name LIKE '%anxiety%' AND downcase_name NOT LIKE 'anxiety%dental' AND downcase_name NOT LIKE 'anxiety reduction, elective cesarean section'
...
SELECT anxiety_studies.nct_id, reported_events.id, subjects_affected
FROM anxiety_studies 
LEFT JOIN reported_events
ON anxiety_studies.nct_id = reported_events.nct_id;
```

```{r}
write.csv(anxiety_ae_key, "/Users/nataliecortopassi/Library/Mobile Documents/com~apple~CloudDocs/510 - HIDS/Adverse Events/ae_key_dfs/anxiety_ae_key.csv")
```    

```{sql connection=con}
SELECT COUNT(DISTINCT nct_id)
FROM anxiety_study_key_V2;
```    
...
```{sql connection=con}
DROP TABLE IF EXISTS anxiety_event_table;
```
...
WITH event_info AS (
    ...
    SELECT anxiety_study_key_V2.nct_ID
    FROM anxiety_study_key_V2
)
 SELECT anxiety_key.nct_id, subjects_affected_total, event_type
 INTO TEMP anxiety_event_table
 FROM anxiety_key 
 LEFT JOIN event_info
 ON anxiety_key.nct_id = event_info.nct_id
``` 

```{sql connection=con, output.var="anxiety_event_table"}
SELECT * 
FROM anxiety_event_table;
```

```{r}
write.csv(anxiety_event_table, "/Users/nataliecortopassi/Library/Mobile Documents/com~apple~CloudDocs/510 - HIDS/anxiety_df/anxiety_event_table.csv")
```
    
#### Select Features and Columns to create target outcome (percentage dropped) - V2
Create table that matches the nct_id found above (anxiety studies) and add features from other aact tables
...
```{sql connection=con}
DROP TABLE IF EXISTS V2_anxiety;
```

```{sql connection=con}
WITH anxiety_list_key AS (
  SELECT *
  FROM anxiety_study_key_V2 
...
SELECT anxiety_list_key.nct_id, anxiety_list_key.enrollment_total, anxiety_list_key.drop_count_total, study_info.study_type, study_info.phase AS study_phase, study_info.number_of_arms, study_info.has_expanded_access, study_info.has_dmc, calculated_info.number_of_facilities,  calculated_info.actual_duration AS study_duration_months, calculated_info.number_of_sae_subjects, calculated_info.minimum_age_num, design_info.intervention_model AS intervention_model_type, design_info.allocation AS allocation_type, design_info.masking AS masking_type, eligibility_info.gender AS study_gender_eligibility, eligibility_info.healthy_volunteers AS health_status_eligibility
    INTO TEMP V2_anxiety
    FROM anxiety_list_key
    LEFT JOIN enrolled_count_list
    ON anxiety_list_key.nct_id = enrolled_count_list.nct_id
    LEFT JOIN dropped_count_list
    ON anxiety_list_key.nct_id = dropped_count_list.nct_id
    LEFT JOIN study_info
    ON anxiety_list_key.nct_id = study_info.nct_id
    LEFT JOIN calculated_info
    ON anxiety_list_key.nct_id = calculated_info.nct_id
    LEFT JOIN country_info
    ON anxiety_list_key.nct_id = country_info.nct_id
    LEFT JOIN design_info
    ON anxiety_list_key.nct_id = design_info.nct_id
    LEFT JOIN eligibility_info
    ON anxiety_list_key.nct_id = eligibility_info.nct_id
    LEFT JOIN intervention_info
    ON anxiety_list_key.nct_id = intervention_info.nct_id
    LEFT JOIN reported_events_info
    ON anxiety_list_key.nct_id = reported_events_info.nct_id
    GROUP BY anxiety_list_key.nct_id, enrollment_total, drop_count_total, study_type, study_phase, number_of_arms, has_expanded_access, has_dmc, number_of_facilities, study_duration_months, number_of_sae_subjects, minimum_age_num, intervention_model_type, allocation_type, masking_type, study_gender_eligibility, health_status_eligibility;
```

```{sql connection=con, output.var="V2_anxiety"}
SELECT * 
FROM V2_anxiety;
```

```{r}
write.csv(V2_anxiety, "/Users/nataliecortopassi/Library/Mobile Documents/com~apple~CloudDocs/510 - HIDS/anxiety_df/V2_anxiety.csv")
``` 

#### Create treatment_type table
```{sql connection=con, output.var="anxiety_treatment_type"}
SELECT V2_anxiety.nct_id, interventions.intervention_type
FROM V2_anxiety
LEFT JOIN ctgov.interventions
ON V2_anxiety.nct_id = interventions.nct_id
```
...
```{r}
write.csv(anxiety_treatment_type, "/Users/nataliecortopassi/Library/Mobile Documents/com~apple~CloudDocs/510 - HIDS/anxiety_df/anxiety_treatment_type.csv")
```    

#### Select dropout reasons
Create table of dropout reasons with nct_id associated with anxiety studies
```{sql connection=con, output.var="anxiety_dropout_reason"}
SELECT drop_withdrawals.reason AS dropout_reason, V2_anxiety.nct_id
FROM V2_anxiety
LEFT JOIN ctgov.drop_withdrawals
ON V2_anxiety.nct_id = drop_withdrawals.nct_id;
```
    
```{r}
write.csv(anxiety_dropout_reason, "/Users/nataliecortopassi/Library/Mobile Documents/com~apple~CloudDocs/510 - HIDS/anxiety_df/anxiety_dropout_reason.csv")
```    
